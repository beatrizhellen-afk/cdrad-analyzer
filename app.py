...
st.sidebar.write("Ajuste as barras para remover os textos e isolar apenas os quadrados da matriz.")
# Barras de rolagem definindo a porcentagem de corte (0% a 30%)
margem_sup = st.sidebar.slider("Cortar Topo (%)", 0, 30, 10)
margem_inf = st.sidebar.slider("Cortar Base (%)", 0, 30, 10)
margem_esq = st.sidebar.slider("Cortar Esquerda (%)", 0, 30, 10)
margem_dir = st.sidebar.slider("Cortar Direita (%)", 0, 30, 10)
...
if arquivo_dicom is not None:
    ...
    if rotacionar:
        imagem_bruta = np.rot90(imagem_bruta)
    
    # Normalização e Realce (CLAHE)
    ...
    # A imagem que será analisada agora é apenas o "miolo" recortado
    ...
    with col1:
        ...
        # A grade 15x15 agora é calculada em cima do recorte, não da imagem toda
        ...
        # Mostramos apenas a ROI na tela
        ...
        for i in range(15): 
            for j in range(15): 
                ...
                # Usando o limiar de sensibilidade dinâmico
