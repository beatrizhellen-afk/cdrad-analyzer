...
# NOVA FUNCIONALIDADE: Rotação Livre (-180 a +180 graus)
# Com passos de 0.5 graus para ajuste fino se estiver "torta".
angulo_rotacao = st.sidebar.slider("🔄 Alinhamento (Rotação Livre)", -180.0, 180.0, 0.0, 0.5)

st.sidebar.subheader("2. Recorte da Matriz (ROI)")
st.sidebar.write("Ajuste as barras para remover os textos e isolar apenas os furos da matriz.")
...
if arquivo_dicom is not None:
    ...
    if angulo_rotacao != 0.0:
        ...
        # Aplica a rotação (Warp Affine). BORDER_REPLICATE para não criar bordas pretas.
    ...
    # CLAHE para realce de contraste (depois de alinhar)
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
    imagem_realcada = clahe.apply(imagem_processada)

    # --- APLICANDO O RECORTE (ROI) NA IMAGEM ALINHADA ---
    ...
    with col2:
        ...
        # Cálculos de Detalhe e Contraste
        indices_detectados = np.where(matriz_deteccao == 1)
        if len(indices_detectados[1]) > 0:
            menor_dia = dimensoes[np.min(indices_detectados[1])] # Detalhe (Colunas)
            menor_prof = dimensoes[np.min(indices_detectados[0])] # Contraste (Linhas)
        else:
            menor_dia, menor_prof = 0, 0

        # --- NOVAS MÉTRICAS COM TERMINOLOGIA CORRETA ---
        met_res_esp, met_threshold = st.columns(2)
        with met_res_esp:
            st.metric("Resolução Espacial Limite (Detalhe)", f"{menor_dia} mm")
        with met_threshold:
            st.metric("Threshold Baixo Contraste (Profundidade)", f"{menor_prof} mm")
